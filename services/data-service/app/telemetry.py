"""Instrumentacion manual de data-service (modulo A del proyecto integrador).
...
tracer = trace.get_tracer("otel-lab.catalog", INSTRUMENTATION_VERSION)
meter = metrics.get_meter("otel-lab.catalog", INSTRUMENTATION_VERSION)

# --- Metricas de negocio (T1.5) ---------------------------------------------

catalog_lookups = meter.create_counter(
    name="catalog_lookups",
    unit="1",
    description="Consultas al catalogo, por resultado (hit / miss / error)",
)

catalog_query_duration = meter.create_histogram(
    name="catalog_query_duration_ms",
    unit="ms",
    description="Duracion de la consulta a Cloud SQL, en milisegundos",
)
